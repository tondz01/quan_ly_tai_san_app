...
class TypeCcdcProvider with ChangeNotifier {
  ...
  TypeCcdc? get dataDetail => _dataDetail;
  ...
  List<TypeCcdc>? _data;
  List<TypeCcdc>? _filteredData;
  List<TypeCcdc>? _dataPage;
  TypeCcdc? _dataDetail;
      ...
      _filteredData =
          _data!.where((item) {
            return (item.id?.toLowerCase().contains(searchLower) ?? false) ||
                ...
                (item.idLoaiCCDC?.toLowerCase().contains(searchLower) ?? false);
          }).toList();
    } else {
      _filteredData = _data!;
    }
  ...
  void getList(BuildContext context) {
    _isLoading = true;
    Future.microtask(() {
      context.read<TypeCcdcBloc>().add(GetListTypeCcdcEvent(context));
    });
  }

  getListSuccess(BuildContext context, GetListTypeCcdcSuccessState state) {
    _error = null;
  ...
  void onChangeDetail(TypeCcdc? item) {
    if (item != null) {
      _dataDetail = item;
      _isCreate = false;
    } else {
  ...
  void createSuccess(BuildContext context, CreateTypeCcdcSuccessState state) {
  ...
  void updateSuccess(BuildContext context, UpdateTypeCcdcSuccessState state) {
    onCloseDetail(context);
  ...
  void deleteSuccess(BuildContext context, DeleteTypeCcdcSuccessState state) {
    onCloseDetail(context);
  ...
  void getListFailed(BuildContext context, GetListTypeCcdcFailedState state) {
    _error = state.message;
    _isLoading = false;
    notifyListeners();
  }

  void createFailed(BuildContext context, CreateTypeCcdcFailedState state) {
    _error = state.message;
  ...
  Future<Map<String, dynamic>?> insertData(
    ...
    try {
      final result =
          kIsWeb
              ? await TypeCcdcRepository().insertDataFileBytes(
                fileName,
                fileBytes,
              )
              : await TypeCcdcRepository().insertDataFile(filePath);
      ...
      SGLog.debug("TypeCcdcProvider", ' Error uploading file: $e');
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
