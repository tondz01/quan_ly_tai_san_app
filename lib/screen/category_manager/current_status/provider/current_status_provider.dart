...
class CurrentStatusProvider with ChangeNotifier {
  ...
  CurrentStatus? get dataDetail => _dataDetail;
  ...
  List<CurrentStatus>? _filteredData;
  List<CurrentStatus>? _dataPage;
  CurrentStatus? _dataDetail;
  ...
  void _applyFilters() {
    ...
    if (_searchTerm.isNotEmpty) {
      ...
      _filteredData =
          _data!.where((item) {
            return (item.id?.toString().toLowerCase().contains(searchLower) ?? false) ||
                (item.tenHTKT?.toLowerCase().contains(searchLower) ?? false);
          }).toList();
    } else {
      _filteredData = _data!;
    }
  ...
  void getList(BuildContext context) {
    ...
    Future.microtask(() {
      ...
      context.read<CurrentStatusBloc>().add(GetListCurrentStatusEvent(context));
    });
  }

  getListSuccess(BuildContext context, GetListCurrentStatusSuccessState state) {
    _error = null;
    if (state.data.isEmpty) {
      ...
      _data = state.data;
      AccountHelper.instance.clearCurrentStatus();
      AccountHelper.instance.setCurrentStatus(_data ?? []);
  ...
  void onChangeDetail(CurrentStatus? item) {
    if (item != null) {
      _dataDetail = item;
      _isCreate = false;
    } else {
  ...
  void createSuccess(BuildContext context, CreateCurrentStatusSuccessState state) {
  ...
  void updateSuccess(BuildContext context, UpdateCurrentStatusSuccessState state) {
    onCloseDetail(context);
  ...
  void deleteSuccess(BuildContext context, DeleteCurrentStatusSuccessState state) {
    onCloseDetail(context);
  ...
  void getListFailed(BuildContext context, GetListCurrentStatusFailedState state) {
    _error = state.message;
    _isLoading = false;
    notifyListeners();
  }

  void createFailed(BuildContext context, CreateCurrentStatusFailedState state) {
    _error = state.message;
  ...
  Future<Map<String, dynamic>?> insertData(
    ...
    try {
      final result =
          kIsWeb
              ? await CurrentStatusRepository().insertDataFileBytes(
                fileName,
                fileBytes,
              )
              : await CurrentStatusRepository().insertDataFile(filePath);
      ...
      SGLog.debug("CurrentStatusProvider", ' Error uploading file: $e');
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
