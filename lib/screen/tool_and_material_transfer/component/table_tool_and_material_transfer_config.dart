...
class TableToolAndMaterialTransferConfig {
        ...
        builder: (item) {
          final ten =
              AccountHelper.instance
                  ...
                  ?.tenPhongBan ??
              '';
          return TableCellData(widget: Text(ten));
        },
      ),
      ColumnDefinition(
        config: TableColumnData.select(
    ...
    showConfirmDialog(
      ...
      onConfirm: () {
        ...
        context.read<ToolAndMaterialTransferBloc>().add(
          SendToSignerTAMTEvent(context, notShared),
        );
      },
    );
  }

  static List<ToolAndMaterialTransferDto> getNotSharedAndNotify(
    BuildContext context,
    List<ToolAndMaterialTransferDto> items,
  ) {
    if (items.isEmpty) {
  ...
  static String getStatusDocumentName(ToolAndMaterialTransferDto item) {
    String statusName =
  ...
  static bool canSign(List<ToolAndMaterialTransferDto> items) {
  ...
  static bool isCheckShowShare(List<ToolAndMaterialTransferDto> items) {
    if (items.isEmpty) {
      return false;
    }
  ...
  static bool isCheckShowDelete(ToolAndMaterialTransferDto item) {
    final userInfo = AccountHelper.instance.getUserInfo();
