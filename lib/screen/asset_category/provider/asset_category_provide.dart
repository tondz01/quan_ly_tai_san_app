...
class AssetCategoryProvider with ChangeNotifier {
  ...
  Future<Map<String, dynamic>?> insertData(
    ...
    try {
      final result =
          kIsWeb
              ? await AssetCategoryRepository().insertDataFileBytes(
                fileName,
                fileBytes,
              )
              : await AssetCategoryRepository().insertDataFile(filePath);
      ...
      SGLog.debug("AssetCategoryProvider", ' Error uploading file: $e');
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
