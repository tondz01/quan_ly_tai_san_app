...
class AssetHandoverPagesViewer {
  ...
  static void showPopup(BuildContext context, String idBienBanDieuChuyen) {
      ...
      builder: (BuildContext context) {
        return Dialog(
          ...
          child: Container(
            ...
            child: _ToolAndSuppliesHandoverPagesViewerContent(
              idBienBanDieuChuyen: idBienBanDieuChuyen,
            ),
          ),
        );
      },
...
class _ToolAndSuppliesHandoverPagesViewerContent extends StatefulWidget {
  final String idBienBanDieuChuyen;

  const _ToolAndSuppliesHandoverPagesViewerContent({
    required this.idBienBanDieuChuyen,
  });
...
class _ToolAndSuppliesHandoverPagesViewerContentState
  ...
  final AssetHandoverRepository _repository = AssetHandoverRepository();
  final AssetHandoverProvider _provider = AssetHandoverProvider();

  List<AssetHandoverDto> _listHandover = [];
  Map<String, List<DetailAssetHandoverDto>> detailsMap =
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final response = await _repository.getDataWithPagination(
        ...
        9999,
        widget.idBienBanDieuChuyen, // searchTerm = id biên bản điều chuyển
      ...
      if (response['status_code'] == 200) {
        ...
        _listHandover =
            data
                .cast<AssetHandoverDto>()
                .where(
                  (item) => item.lenhDieuDong == widget.idBienBanDieuChuyen,
                )
                .toList();
  ...
  Future<void> _loadDetailsForPage(int index) async {
    ...
    if (item.chiTietBanGiaoTaiSan != null &&
        item.chiTietBanGiaoTaiSan!.isNotEmpty) {
      if (mounted) {
        setState(() {
          detailsMap[itemId] = item.chiTietBanGiaoTaiSan!;
        });
      }
      return;
    }
                            ...
                            child: Padding(
                              ...
                              child: _PageViewItem(
                                ...
                                itemsDetail:
                                    ...
                                    _listHandover[index].chiTietBanGiaoTaiSan ??
                                    [],
...
class _PageViewItem extends StatefulWidget {
  ...
  final List<DetailAssetHandoverDto> itemsDetail;
  ...
  Widget build(BuildContext context) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Biên bản: ${widget.item.banGiaoTaiSan ?? widget.item.id ?? ""}',
    ...
    return RepaintBoundary(
      child: _AssetHandoverPreviewContent(
        item: widget.item,
        itemsDetail: widget.itemsDetail,
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onPanUpdate: (details) {
        ...
        if (_isHorizontalDrag && widget.pageController.hasClients && _initialPage != null) {
          // Tính offset dựa trên drag distance
          // Drag sang trái (deltaX < 0) = next page, drag sang phải (deltaX > 0) = previous page
          final normalizedOffset = -deltaX / screenWidth; // Đảo ngược để drag trái = tăng page
          final newPage = (_initialPage! + normalizedOffset).clamp(0.0, (widget.totalPages - 1).toDouble());
