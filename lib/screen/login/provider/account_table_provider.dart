...
final accountTableProvider = StateNotifierProvider.autoDispose<
  AccountTableProvider,
  GenericTableState<UserInfoDTO>
>((ref) {
  final repository = AuthRepository();
  return AccountTableProvider(repository);
});

class AccountTableProvider extends TableNotifier<UserInfoDTO> {
  final AuthRepository repository;
  int totalItems = 0;
  String _currentSearchTerm = '';
  AccountTableProvider(this.repository);
  // FIXED: Signature đúng với named parameters
  @override
  void initialize({
    required Map<String, double> columnWidths,
    required dynamic Function(UserInfoDTO item, int columnIndex) valueGetter,
    int itemsPerPage = 20,
  }) {
    super.initialize(
  ...
  @override
  Future<List<UserInfoDTO>> generateData() async {
