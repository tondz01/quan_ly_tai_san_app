      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
            ...
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: const ExempleScreen1()),
          ),
          GoRoute(
            path: AppRoute.exemple2.path,
            name: AppRoute.exemple2.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: const ExempleScreen2()),
          ),
          GoRoute(
            path: AppRoute.exemple3.path,
            name: AppRoute.exemple3.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: const ExempleScreen3()),
          ),
          GoRoute(
            path: AppRoute.exemple4.path,
            name: AppRoute.exemple4.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: const ExempleScreen4()),
          ),
          GoRoute(
            path: AppRoute.toolsAndSupplies.path,
            name: AppRoute.toolsAndSupplies.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: const ToolsAndSuppliesView()),
          ),
          GoRoute(
            ...
            pageBuilder:
                (context, state) => NoTransitionPage(
                  ...
                  child: AssetTransferView(
                    typeAssetTransfer:
                        state.extra != null && state.extra is String ? int.tryParse(state.extra as String) ?? 0 : 0,
                  ),
                ),
          ),
          GoRoute(
            ...
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: const AssetHandoverView()),
          ),
          GoRoute(
            path: AppRoute.staffManager.path,
            name: AppRoute.staffManager.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: StaffManager()),
          ),
          GoRoute(
            path: AppRoute.projectManager.path,
            name: AppRoute.projectManager.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: ProjectManager()),
          ),
          GoRoute(
            path: AppRoute.departmentManager.path,
            name: AppRoute.departmentManager.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: DepartmentManager()),
          ),
          GoRoute(
            path: AppRoute.capitalSource.path,
            name: AppRoute.capitalSource.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: CapitalSourceManager()),
          ),
          GoRoute(
            path: AppRoute.assetManager.path,
            name: AppRoute.assetManager.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: AssetManager()),
          ),
          GoRoute(
          ...
          GoRoute(
            ...
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: AssetGroupView()),
          ),
          GoRoute(
            path: AppRoute.assetManagement.path,
            name: AppRoute.assetManagement.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: AssetManagementView()),
          ),
          GoRoute(
            path: AppRoute.assetCategory.path,
            name: AppRoute.assetCategory.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: AssetCategoryView()),
          ),

          GoRoute(
            path: AppRoute.fixedAssetRegister.path,
            name: AppRoute.fixedAssetRegister.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: SoTaiSanCoDinh()),
          ),
          GoRoute(
            path: AppRoute.fixedAssetRegisterCircular200.path,
            name: AppRoute.fixedAssetRegisterCircular200.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: SoTaiSanCoDinh200()),
          ),
          GoRoute(
            path: AppRoute.trackingRecord.path,
            name: AppRoute.trackingRecord.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: SoTheoDoi()),
          ),
          GoRoute(
            path: AppRoute.account.path,
            name: AppRoute.account.name,
            pageBuilder: (context, state) => NoTransitionPage(key: state.pageKey, child: AccountView()),
          ),
          GoRoute(
            ...
            pageBuilder:
                (context, state) =>
                    NoTransitionPage(key: state.pageKey, child: ReportScreen(idCongty: 'CT001', loai: 1)),
          ),
          GoRoute(
            ...
            pageBuilder:
                (context, state) =>
                    NoTransitionPage(key: state.pageKey, child: ReportScreen(idCongty: 'CT001', loai: 2)),
          ),
          GoRoute(
            ...
            pageBuilder:
                (context, state) =>
                    NoTransitionPage(key: state.pageKey, child: ReportScreen(idCongty: 'CT001', loai: 3)),
          ),
        ],
      ),
    ],
